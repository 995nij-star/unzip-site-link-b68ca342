...
import { useClipByShortCode, useClipActions, useClipComments } from "@/hooks/useClips";
...
import { ArrowLeft, Heart, MessageCircle, Send, Loader2, Film, Share2, Flag, Eye } from "lucide-react";
import { toast } from "@/hooks/use-toast";
import { format } from "date-fns";
import { ReportClipDialog } from "@/components/clips/ReportClipDialog";

export default function ClipShortRedirect() {
  const { code } = useParams<{ code: string }>();
  const { clip, isLoading } = useClipByShortCode(code || "");
  const { toggleLike, addComment } = useClipActions();
  const { user } = useAuth();
  const [commentText, setCommentText] = useState("");
  const { checkContent } = useContentModeration();
  const { comments, isLoading: commentsLoading } = useClipComments(clip?.id || "");
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-background/90 border-b border-primary/20">
        <div className="container mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <span className="text-lg font-orbitron font-bold text-foreground truncate flex-1">{clip.title}</span>
        ...
        <div className="premium-card rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-3 p-4 pb-2">
            ...
            <div className="flex-1">
              <Link to={`/creator/${clip.user_id}`} className="font-rajdhani font-semibold text-foreground hover:text-primary transition-colors">
          ...
          <div className="p-4 flex items-center gap-4">
            ...
            <button onClick={handleShare} className="flex items-center gap-1.5 text-sm font-rajdhani text-muted-foreground hover:text-foreground transition-colors">
              <Share2 className="w-5 h-5" />
            </button>
            {user && (
              <button onClick={() => setReportOpen(true)} className="flex items-center gap-1.5 text-sm font-rajdhani text-muted-foreground hover:text-destructive transition-colors ml-auto">
      ...
      <ReportClipDialog clipId={clip.id} open={reportOpen} onOpenChange={setReportOpen} />
    </div>
  );
}
