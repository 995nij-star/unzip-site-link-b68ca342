...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (user_id === caller.id) {
      return new Response(JSON.stringify({ error: 'Cannot reset your own password through admin panel' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
